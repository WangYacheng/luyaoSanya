...
public class DataDownloadService
{
    ...
    public Task DownloadOkxAsync(IProgress<DownloadProgress> progress)
    {
        return DownloadExchangeDailyAsync(_okxService, "OKX", progress);
    }

    public Task DownloadBinanceAsync(IProgress<DownloadProgress> progress)
    {
        return DownloadExchangeDailyAsync(_binanceService, "Binance", progress);
    }
    ...
    {
        var symbols = (await exchangeService.GetSymbolsAsync(MarketType.Swap, "USDT")).ToList();
        if (symbols.Count == 0)
        {
